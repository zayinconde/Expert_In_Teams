...
// ============================================================================
// FFT SETUP FOR BASS EXTRACTION
// ============================================================================
#define SAMPLES 128              // Must be power of 2 (128 is good balance)
#define SAMPLING_FREQUENCY 44100  // Match audio sample rate
...
volatile double bassEnergy = 0;
volatile double midEnergy = 0;

// ============================================================================
// BLUETOOTH AUDIO WITH ADVANCED MUSIC REACTIVE MODE
// ============================================================================
I2SStream i2s;
...
class HapticSmoother {
  private:
    double smoothFactor = 0.7;  // Adjust responsiveness
    double currentValue = 0;
  
  public:
    int32_t smooth(double newValue) {
...
// Audio data callback with FFT analysis
void audio_data_callback(const uint8_t *data, uint32_t len) {
  // Always pass audio through to speakers
  i2s.write(data, len);
  ...
  for (int i = 0; i < numSamples && sampleIndex < SAMPLES; i++) {
    vReal[sampleIndex] = (double)samples[i];
    vImag[sampleIndex] = 0;
    sampleIndex++;
  }
  ...
  if (sampleIndex >= SAMPLES) {
    ...
    // At 44.1kHz with 128 samples: each bin = 344.5 Hz
    // Bins 0-1 = 0-689 Hz (includes bass range)
    double bass = 0;
    for (int i = 0; i < 2; i++) {  // First 2 bins cover bass
      bass += vReal[i];
    }
    
    // Extract mid energy (250-2000 Hz) for comparison
    double mid = 0;
    for (int i = 2; i < 6; i++) {  // Next 4 bins
      mid += vReal[i];
    }
...
// ============================================================================
// ADVANCED MUSIC MODE WITH FFT
// ============================================================================
void updateMusicHaptics() {
  if (!musicModeActive) return;
  
  // Map bass energy to haptic amplitude
  // Typical bass energy range: 0-500000 (adjust based on testing)
  int32_t mappedLRA = map(constrain(bassEnergy, 0, 300000), 0, 300000, 0, 127);
  int32_t mappedERM = map(constrain(bassEnergy, 0, 300000), 0, 300000, 0, 127);
  
  // Apply smoothing for controlled response
  mappedLRA = lraSmoother.smooth(mappedLRA);
  mappedERM = ermSmoother.smooth(mappedERM);
  
  // Apply threshold to avoid buzz
  if (mappedLRA < 30) mappedLRA = 0;
  if (mappedERM < 30) mappedERM = 0;
...
void runFullDemo() {
  ...
  Serial.println("--- Part 3b: ERM Attempt ---");
  for (int i = 0; i < 15; i++) {
    setAmp127(ADDR_ERM, 127);
    delay(150);
    setAmp127(ADDR_ERM, 0);
  ...
  Serial.println("--- Part 4b: ERM Heartbeat ---");
  for (int i = 0; i < 3; i++) {
    setAmp127(ADDR_ERM, 127);
    delay(200);
    setAmp127(ADDR_ERM, 0);
...
void handleButton() {
  ...
  if (currentState && !buttonPressed) {
    ...
    if (holdTime < 2000) {
      ...
      if (musicModeActive) {
        Serial.println("\n♪ MUSIC MODE ENABLED (FFT) ♪");
        Serial.println("Haptics will pulse with bass frequencies!");
        lraSmoother.reset();
        ermSmoother.reset();
      } else {
        Serial.println("\n♪ MUSIC MODE DISABLED");
...
void setup() {
  ...
  Serial.println("Configuring ERM at 0x48...");
  clearIrqWriteOnly(ADDR_ERM);
  writeReg(ADDR_ERM, REG_OPERATIONMODE, MODE_DRO);
  delay(100);

  Serial.println("Starting Bluetooth with FFT Audio Analysis");
  ...
  Serial.println("  • Short press = Toggle FFT music mode");
  Serial.println("  • Hold 2 sec  = Run demo");
  Serial.println("\nFFT bass extraction enabled!");
  Serial.println("Play music with strong bass\n");
...
void loop() {
  ...
  delay(5);  // Faster update for FFT mode
}
