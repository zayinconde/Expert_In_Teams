...
bool writeRegisterDirect(uint8_t reg, uint8_t val);  // Just declaration, no body
...
// added to manually enable the device -----------------------------------------------------------
bool Haptic_Driver::setActive(bool on) {
  // TOP_CTL1 bit3 is the ACTIVE/ENABLE bit on DA7280 (mode bits are 2:0, play/GO is bit4).
  // Mask 0xF7 keeps all bits except bit3, then we set bit3 to on/off.
  return _writeRegister(TOP_CTL1, 0xF7, on ? 1 : 0, 3);
}
