#include <Arduino.h> 
#include <Wire.h> // for i2c
#include <Haptic_Driver.h> // for da7280 library

Haptic_Driver haptic;          // address fixed to 0x4A by this lib


//function for showing status messages
static void assertMsg(bool ok, const char* msg) {
  Serial.print(msg); 
  Serial.println(ok ? " OK" : " FAIL");
}

struct regWrite {
  uint8_t reg;
  uint8_t val;
};

regWrite lraTuningData[] = {
  {0x03, 0x00},
  {0x04, 0x00},
  {0x05, 0x00},
  {0x07, 0x00},
  {0x08, 0x40},
  {0x0A, 0x22},
  {0x0B, 0x3C},
  {0x0C, 0x6B},
  {0x0D, 0x6B},
  {0x0E, 0x1F},
  {0x0F, 0x00},
  {0x10, 0x98},
  {0x13, 0x18},
  {0x14, 0x11},
  {0x15, 0x03},
  {0x16, 0x40},
  {0x17, 0x81},
  {0x1C, 0x0E},
  {0x1D, 0x20},
  {0x1E, 0x03},
  {0x1F, 0x20},
  {0x20, 0x43},
  {0x22, 0x00},
  {0x23, 0x36},
  {0x24, 0x08},
  {0x25, 0x61},
  {0x26, 0xB4},
  {0x27, 0xEC},
  {0x28, 0x00},
  {0x29, 0x00},
  {0x2A, 0x08},
  {0x2B, 0x10},
  {0x2D, 0x80},
  {0x48, 0x1D},
  {0x49, 0x04},
  {0x4C, 0x02},
  {0x5F, 0x0E},
  {0x60, 0x9D},
  {0x62, 0x6F},
  {0x6E, 0x00},
  {0x81, 0x04},
  {0x83, 0x00},
  {0x84, 0x00},
  {0x85, 0x00},
  {0x86, 0x00},
  {0x87, 0x00},
  {0x88, 0x00},
  {0x89, 0x00},
  {0x8A, 0x00},
  {0x8B, 0x00},
  {0x8C, 0x00},
  {0x8D, 0x00},
  {0x8E, 0x00},
  {0x8F, 0x00},
  {0x90, 0x00},
  {0x91, 0x00},
  {0x92, 0x00},
  {0x93, 0x00},
  {0x94, 0x00},
  {0x95, 0x00},
  {0x96, 0x00},
  {0x97, 0x00},
  {0x98, 0x00},
  {0x99, 0x00},
  {0x9A, 0x00},
  {0x9B, 0x00},
  {0x9C, 0x00},
  {0x9D, 0x00},
  {0x9E, 0x00},
  {0x9F, 0x00},
  {0xA0, 0x00},
  {0xA1, 0x00},
  {0xA2, 0x00},
  {0xA3, 0x00},
  {0xA4, 0x00},
  {0xA5, 0x00},
  {0xA6, 0x00},
  {0xA7, 0x00},
  {0xA8, 0x00},
  {0xA9, 0x00},
  {0xAA, 0x00},
  {0xAB, 0x00},
  {0xAC, 0x00},
  {0xAD, 0x00},
  {0xAE, 0x00},
  {0xAF, 0x00},
  {0xB0, 0x00},
  {0xB1, 0x00},
  {0xB2, 0x00},
  {0xB3, 0x00},
  {0xB4, 0x00},
  {0xB5, 0x00},
  {0xB6, 0x00},
  {0xB7, 0x00},
  {0xB8, 0x00},
  {0xB9, 0x00},
  {0xBA, 0x00},
  {0xBB, 0x00},
  {0xBC, 0x00},
  {0xBD, 0x00},
  {0xBE, 0x00},
  {0xBF, 0x00},
  {0xC0, 0x00},
  {0xC1, 0x00},
  {0xC2, 0x00},
  {0xC3, 0x00},
  {0xC4, 0x00},
  {0xC5, 0x00},
  {0xC6, 0x00},
  {0xC7, 0x00},
  {0xC8, 0x00},
  {0xC9, 0x00},
  {0xCA, 0x00},
  {0xCB, 0x00},
  {0xCC, 0x00},
  {0xCD, 0x00},
  {0xCE, 0x00},
  {0xCF, 0x00},
  {0xD0, 0x00},
  {0xD1, 0x00},
  {0xD2, 0x00},
  {0xD3, 0x00},
  {0xD4, 0x00},
  {0xD5, 0x00},
  {0xD6, 0x00},
  {0xD7, 0x00},
  {0xD8, 0x00},
  {0xD9, 0x00},
  {0xDA, 0x00},
  {0xDB, 0x00},
  {0xDC, 0x00},
  {0xDD, 0x00},
  {0xDE, 0x00},
  {0xDF, 0x00},
  {0xE0, 0x00},
  {0xE1, 0x00},
  {0xE2, 0x00},
  {0xE3, 0x00},
  {0xE4, 0x00},
  {0xE5, 0x00},
  {0xE6, 0x00},
  {0xE7, 0x00}
};



void setup(){
Serial.begin(115200);
delay(200);
Wire.begin(21, 22); // SDA, SCL
Wire.setClock(400000); // 400kHz I2C //DA7280 supports up to 1MHz

if(haptic.begin(Wire)){
    Serial.println("Haptic Driver Found!");
  } else {
    Serial.println("Haptic Driver Not Found!");
    while(1); // halt
  }

  haptic.setActuatorType(LRA_TYPE); // set motor type to ERM

  Serial.println("Setting I2C Operation");
  //haptic.setOperationMode(DRO_MODE); // set to I2C control mode
  haptic.setOperationMode(DRO_MODE); // set to real-time mode
  Serial.println("Ready boiiii!");

  //need to create a way to writing LRA tuning to DA7280 here
  for (int i = 0; i < sizeof(lraTuningData)/sizeof(lraTuningData[0]); i++) {
  bool ok = haptic.writeRegisterDirect(lraTuningData[i].reg, lraTuningData[i].val);
  Serial.print("Write 0x");
  Serial.print(lraTuningData[i].reg, HEX);
  Serial.print(" = 0x");
  Serial.print(lraTuningData[i].val, HEX);
  Serial.println(ok ? " OK" : " FAIL");

  

  haptic.enableFreqTrack(true); // disable freq track for coin ERM

  //haptic.setActuatorNOMVolt(2.7f);
  //haptic.setActuatorABSVolt(3.0f);
  haptic.setActuatorIMAX(0.50f);
  haptic.setActuatorImpedance(6.6f);

  //haptic.enableRapidStop(true);
  //haptic.enableAmpPid(true);


  
}



}

void loop() {
  // put your main code here, to run repeatedly:
  haptic.setVibrate(255); // max vibration
  Serial.println("Vibrating at max!");
  delay(1000);
  haptic.setVibrate(0); // stop vibration
  Serial.println("Stopping Vibration");
  delay(1000);

}